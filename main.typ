...
- обеспечить корректную параллельную работу нескольких экземпляров приложения за счет безопасного доступа к файлам
- спроектировать иерархию типов учетных записей с дифференциацией пользовательских полномочий (администратор, продавец, покупатель)
- разработать интуитивно понятный графический интерфейс пользователя (GUI)
- провести тестирование работоспособности приложения на операционных системах семейств "Windows" и "Linux"

Особое внимание в работе уделяется проектированию отказоустойчивой файловой системы хранения данных. Архитектура должна гарантировать целостность информации при одновременном открытии программы от лица разных учетных записей (например, когда администратор добавляет категорию, а покупатель в это же время просматривает каталог).